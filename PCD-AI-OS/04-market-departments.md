...
4. **Inputs.** The portfolio inbox `jeff@parentcoachdesk.com`, labeled by Cloudflare Email Routing. `PCD-OPERATING-MANUAL.md` SOP S12. `About Me/About Me.txt` and `About Me/Anti AI Writing.txt`, read before drafting a word. Vera's workflow, so deletion requests are correctly handed off, not triaged as ordinary mail.
