...
**Maintenance mode, updated 2026-08-01:** the automatic August-through-November calendar freeze is removed. Jeff is working PCD nightly through football season now, so the "founder unavailable four months a year" assumption behind the old freeze no longer holds. `PCD_MAINTENANCE_MODE` remains as a manual operator toggle he can flip to pause writes deliberately; the S4 deletion watch stays exempt and the operator override still cannot suppress it. The two 2026 success metrics are the Chain Reaction manuscript and UPS football finishing 4th in conference, and this design should assume the founder has less time in-season, not zero time.
...
10. **Maintenance mode.** A global PCD_MAINTENANCE_MODE toggle degrades every agent to report-only when Jeff turns it on, with a named bypass list (S4 deletion watch, security incidents, domain/payment failures). There is no automatic calendar trigger; this is operator-controlled only.

    *Enforcement status (2026-08-01 update).* The August-through-November auto-freeze in `src/lib/maintenance-mode.ts` and `worker-cron/src/index.ts` is removed, because Jeff is now working PCD nightly through the season and the founder-unavailable assumption it encoded no longer holds. The bypass list stays code-enforced for the `deletion_opt_out_proposal` write class (`writeHeldDuringMaintenance` and `MAINTENANCE_EXEMPT_WRITE_CLASSES`). The S4 path is never held, and the operator override still cannot suppress it, proven in `tests/maintenance-mode.test.ts`.

    Maintenance mode is code-enforced only where a write flows through a code path: Ranger's S7 autonomous camp writes, held by the `camps-sweep` cron guard when `PCD_MAINTENANCE_MODE` is on. The other eight tasks' report-only posture during a manual maintenance window is prompt-only. Each agent SKILL would need its own check against the toggle and there is no runtime guard behind them, so their report-only behavior stays honor-system, not enforced.
