...
Added 2026-08-06: D1 `activity-radar`, eight tables for the competitor intelligence subsystem (migration `migrations-activity-radar/0016_competitor_intelligence.sql`, access layer `src/lib/intel/store.ts`): `competitors`, `org_tech_signals`, `org_tech_stack`, `org_tech_history`, `intel_runs`, `intel_review_queue`, `org_opportunity_scores`, `intel_fetch_log`. No new personal data collection, no new third party. Full detail in the new section below.
...
## Competitor intelligence tables (added 2026-08-06)

Eight tables in D1 `activity-radar`, all from migration `migrations-activity-radar/0016_competitor_intelligence.sql`. This subsystem fingerprints which club-management, registration, or website software an organization already listed in this database appears to run, and scores that org as a switch target. It never crawls a competitor's own website on an automated run (see `docs/COMPETITOR-INTELLIGENCE-RUNBOOK.md` for the policy). Nothing here is submitted by a person; every row is either a system-tracked vendor catalog or a fact the crawler observed on an organization's own public website.

| Table | What it holds | Personal data | Retention |
|---|---|---|---|
| `competitors` | The catalog of vendors tracked (SportsGravy, TeamSnap, and so on): display name, domain, category, status. | None. Vendor company data, not personal data. | Indefinite; a small reference table. |
| `org_tech_signals` | Append-only evidence log. One row per matched fingerprint pattern: which competitor, which pattern, a truncated matched-text snippet, the source URL. | Business data about an organization's own public website, not a person. `matched_value` is the literal matched substring for that pattern only (a vendor URL fragment, an app-store id, a brand-name phrase), not a page excerpt or free text, and is separately capped at 500 characters in app code (`MAX_MATCHED_VALUE_LENGTH`). The cap bounds size, not content: today's fingerprint patterns target vendor domains, app-store ids, and brand phrases, never a name field, so no pattern currently captures a person's name. A future pattern written broadly enough against page text could in principle match text that includes a name; that risk is architectural, not observed, and is the reason this row is called out explicitly rather than assumed clean. |Indefinite. This is the audit trail behind every stack detection; rows are never updated or deleted. |
| `org_tech_stack` | Current believed tech stack per organization and category (club management, registration, website, payments, streaming, communications), with a confidence score and review status. | Business data about an organization, keyed to the existing `organizations.id`. No named-person field. | Kept as long as the parent `organizations` row exists. |
| `org_tech_history` | Append-only change log behind `org_tech_stack`: first detected, switched vendor, confidence changed, confirmed, rejected. | Same as `org_tech_stack`; `reviewed_by`/`resolved_by` on the related tables store an admin's email (the operator, not a data subject), same pattern as every other admin-receipted table in this app. | Indefinite; append-only audit trail. |
| `intel_runs` | One row per sweep or crawl job: type, status, who requested and approved it, counts fetched/skipped/found. | Contains the requesting/approving admin's email (operator identity, same category as every other admin-action log in this app, not a third-party data subject). | Indefinite; operational log. |
| `intel_review_queue` | Detections not confident or clean enough to auto-promote, pending a human accept/reject decision. | Same organization-level business data as `org_tech_stack`, plus the resolving admin's email once resolved. | Indefinite. |
| `org_opportunity_scores` | Composite outreach-priority score per organization (migration difficulty, size, tech maturity, switch likelihood, a modeled revenue estimate) with a stored rationale. | None beyond the organization-level business data already covered by `org_tech_stack`. No person, no contact info. | Kept as long as the parent `organizations` row exists; recomputed in place on each rescore. |
| `intel_fetch_log` | Politeness ledger: one row per domain/path ever fetched, with etag/last-modified/robots-allowed state, for conditional requests and rate limiting. | None. Infrastructure/HTTP metadata about a domain, not a person. | Indefinite; small, keyed on `(domain, path)`. |

None of these eight tables collect anything from a site visitor, a parent, or a named individual. They observe an organization's own public website (script tags, link hrefs, page text, meta tags, app-store links) and store what a fingerprint pattern matched, not the page itself. If a future competitor definition ever adds a fingerprint pattern broad enough to capture free text near a person's name (an "our team" page, a staff bio), that pattern needs the same review this map's `org_contacts` entry already got before it ships, since the 500-character cap protects against a runaway row, not against what kind of text ends up in it.
