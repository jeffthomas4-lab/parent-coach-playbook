...
Pick the ~65 slugs with the oldest last-checked date. As of 2026-08-17, the 07-08 through 07-20 pools are fully cleared (all now dated 2026-08-10 or later, except soccer-ball-size4/soccer-ball-size3/soccer-ball-size5-match/soccer-goalie-gloves-youth/soccer-clearance-cleats/etc. — check the table above for exact current dates). The 07-27 pool has 50 slugs remaining (dated 2026-07-27; 15 of its 65 were pulled forward this run for traffic weighting via `placementCount` in `reports/affiliate/lifecycle.json`, top of that list: resistance-bands-set at 22 placements). Next run should pick the remaining **50 slugs still dated 2026-07-27** (oldest remaining), then move to the 08-03 pool for the traffic-weighted top-15 fill. Continue the ~monthly cadence.

**Important for next run:** Watch soccer-shin-guards-ankle-youth (broken/degrading for 4 weeks straight, 07-20 → 08-17) and foam-roller-medium (bounced OOS → resolved → OOS again across three cycles) — both look like candidates the replacement-sourcer keeps sitting on or that keep flapping in and out of stock. Worth asking Jeff whether these should just be swapped preemptively rather than re-checked again next cycle. Also worth re-verifying whether the `worker-link-checker` Worker situation has changed — as of this run it is still not deployed (not present in the account's live Workers list) and its `link_health` D1 table, while it now exists (681 rows, unlike the 0-table state noted on 08-10), has a `last_checked` max of 2026-07-22 — stale by nearly a month, meaning nothing is populating it on an ongoing basis. Treat hard 404s as in-scope again next run unless Jeff confirms otherwise.

**Note carried from 2026-08-10/08-03:** the pattern of unresolved carry-overs recommends checking whether the replacement-sourcer (Arnie) is actually working `replacement-queue.json` before assuming the backlog is shrinking.
