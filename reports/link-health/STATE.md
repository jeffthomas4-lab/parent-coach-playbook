...
Pick the ~65 slugs with the oldest last-checked date. As of 2026-07-27, the 07-08 and 07-13 pools are both fully cleared (all slugs from those batches now dated 2026-07-27 or 2026-07-20). The 07-18 pool has 50 slugs remaining (dated 2026-07-18; 15 of its 65 were pulled forward this run for traffic weighting). Next run should pick the remaining **50 slugs still dated 2026-07-18** (oldest remaining), then move to the 07-19 pool, then 07-20. Continue the ~monthly cadence. Traffic-weighting proxy used again this run: count of references to each slug across `src/content/**` (guides/articles/pillar pages) as a stand-in for page traffic — no direct analytics access. Consider asking Jeff whether a real analytics source (GSC/GA) can be connected for a better traffic signal.
