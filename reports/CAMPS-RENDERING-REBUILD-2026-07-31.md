...
## 2026-07-31, session 6, the customer-lifecycle worker crash

Once the splitter fix above let both integration suites actually run, `npm test` moved from 3 failing files to 1 dead worker. `tests/customer-lifecycle.integration.test.ts` no longer failed a test. Its vitest fork exited with "Worker exited unexpectedly," and vitest reported the whole file as an error rather than pass or fail. `tests/editorial-records-migration.test.ts`, right next to it, ran 9 tests green in 31 seconds. Both files were now genuinely executing for the first time; the question was why only one process died.

**Diagnosis.** Only two files in this repo touch Miniflare: `tests/customer-lifecycle.integration.test.ts` and `tests/editorial-records-migration.test.ts`, both through the shared `createDisposableOpsDatabase()` helper. Each call spins up a real workerd native subprocess. `customer-lifecycle` holds two of them alive at once: the `beforeAll` database (disposed in `afterAll`, so alive for the whole file) plus a second one created inside the last `it` block for the commerce sub-scenario, live at the same time as the first.

`vitest.config.ts`, the config `npm test` actually runs, has no pool or parallelism settings at all. Vitest's default pool runs test files concurrently across multiple forked processes with no cap tied to this repo. That means under plain `npm test`, `customer-lifecycle`'s two workerd processes could be running at the same moment as `editorial-records-migration`'s own workerd process in a sibling fork, three native runtime processes contending for memory and OS handles with zero isolation. "Worker exited unexpectedly" is the vitest pool reporting that a child process died outright, not that a test timed out or threw, which matches a native-process crash under resource pressure rather than a script bug.

This diagnosis is not a guess invented for this session. The repo's own `vitest.integration.config.ts` already carries the fix and says so directly in a comment: `pool: 'threads', maxWorkers: 1, fileParallelism: false`, with the comment "the disposable D1 suite owns a Miniflare runtime. Keep the integration boundary in one thread so a worker-process crash cannot be reported as a successful partial run on Windows." Whoever built the integration config had already hit and solved this exact class of crash. `vitest.config.ts`, the plain-`npm test` default, never got the same treatment; both Miniflare-owning files still sit in its unrestricted `include`.

**Disposal check, ruled out as the cause but confirmed correct anyway.** The task's leading hypothesis included checking whether the second Miniflare instance in `customer-lifecycle` is disposed on the failure path as well as the success path. Read the file directly: it already wraps the commerce sub-scenario in `try { ... } finally { await commerce.mf.dispose(); }`, so disposal already runs whether the assertions inside pass or throw. The `beforeAll` instance is assigned to `mf` before any insert runs, and `afterAll(async () => mf?.dispose())` covers it regardless of whether the batch insert fails. No missing try/finally exists in this file. `editorial-records-migration.test.ts` only ever creates one Miniflare instance, disposed the same safe way. No code change was needed or made here.

**Fix.** Added an `exclude` list to `vitest.config.ts` naming only the two files that actually own a Miniflare/workerd process, with a comment explaining why. This is the smallest change that removes the concurrent-native-process contention: it does not touch the splitter, `disposable-ops-db.ts`, or either test file's logic, and it does not use `.skip` or drop the suites from running anywhere. Both files still run, and run correctly, under `npm run test:integration` (`vitest.integration.config.ts`), which already pins `pool: 'threads'`, `maxWorkers: 1`, `fileParallelism: false` for exactly this reason. That command is not a local-only convenience: `.github/workflows/ci.yml`'s `build-and-test` job runs it as the "Integration paths" step (line 111) on every pull request and every push to `main`. Plain `npm test` was never run by CI at all; CI's push-time job runs `npm run test:unit:coverage`, which already excludes both files via the existing `integrationTests` list in `test-classification.ts`. So this fix routes the two heavy suites to the one config built and proven to isolate them, the same config CI already exercises, rather than inventing a new location for them.

Deliberately did not exclude the other 9 files in `test-classification.ts`'s `integrationTests` list (`customer-journey-monitor`, `directory-idempotency-rehearsal`, `integrated-failure-isolation`, `middleware`, `trust-migration-rehearsal`, `worker-entry-boundary`, and the 3 `tests/api/admin-trust-*`/`trust-request` files). Checked each for `Miniflare`/`workerd` references directly: none use it. They carry no native-process contention risk, so pulling them out of the default `npm test` run would have been a larger, unjustified behavior change with no bearing on this crash.

**What could not be verified.** `npx vitest run` cannot execute in this sandbox (missing native binding, the same standing limitation noted throughout this file), so the crash could not be reproduced here and the fix could not be confirmed to actually stop it. The diagnosis is built from source and config reading only: the file-level Miniflare/workerd usage, `vitest.config.ts`'s missing pool settings versus `vitest.integration.config.ts`'s existing ones and their explanatory comment, and confirmation that `npm run test:integration` is a real, currently-run CI step rather than a documented-but-dead command.

**What Jeff should watch for on a real run.** `npm test` should now report 197 files (199 minus the two moved out), all green, no worker crash. `npm run test:integration` should report both `customer-lifecycle.integration.test.ts` and `editorial-records-migration.test.ts` passing under the single-threaded pool; if that command also crashes, the cause is not fork-level contention with the rest of the suite (there is none, `fileParallelism: false` in that config already means only one file runs at a time) and points instead at something inside `customer-lifecycle` itself, most likely the two-Miniflare-instances-in-one-file pattern under `maxWorkers: 1`, which this session did not change and did not rule out as a cause under that config's tighter constraints. If it passes clean, this is closed.

File changed: `vitest.config.ts`.

## 2026-08-01, session 7, the August time-bomb tests and the customer-lifecycle 7/8 hang

Two unrelated failures surfaced on `audit/full-standard-2026-07-30` once the clock crossed into August. Both fixed in source; neither could be run in this sandbox.

### Problem 1: ten tests assumed it was never August

`src/lib/maintenance-mode.ts` freezes ordinary writes from `2026-08-01T00:00Z` through November. That freeze is correct and stays untouched. The bug was in the tests: most of `tests/api/cron-camps-sweep.test.ts` and one test in `tests/worker-cron.test.ts` called the real code path with no frozen clock, so they silently relied on the wall clock sitting outside Aug-Nov. The moment UTC crossed into August they all flipped to asserting the maintenance-hold response instead of the sweep-runs response they actually test.

**`tests/api/cron-camps-sweep.test.ts`**, verified the failing test count matches exactly: of 14 tests, 2 auth checks and the CRON_KEY-missing check return before the maintenance check runs, one test already freezes to August 1 to test the hold itself, and one test forces the operator override (true regardless of month) — those 5 are date-safe. The other 9 all called `POST` with no frozen clock and asserted normal-sweep behavior. Froze all 9 to `2026-07-15T12:00:00Z` (July, outside the window) with `vi.useFakeTimers()` / `vi.setSystemTime()` at the top of each test, matching the file's own existing pattern from the frozen-August test. `afterEach` already calls `vi.useRealTimers()`, so no test leaks a frozen clock into a sibling file.

**`tests/worker-cron.test.ts`**: only one of 11 tests was unsafe — `'reports readiness without exposing which secret or variable is missing'` asserts `maintenance_mode: false` unconditionally with no frozen clock, calling `/ready`, which reads `Date.now()` directly. Every other test in the file either passes an explicit `Date.UTC(...)` argument to `runScheduledSweep`/`maintenanceModeActive` or asserts `expect.any(Boolean)`, so they were already date-safe. Froze the one bad test to the same `2026-07-15T12:00:00Z` and added `vi.useRealTimers()` to the file's existing `afterEach` (previously only `vi.restoreAllMocks()`).

**Other clock-dependent tests found, repo-wide.** Grepped `tests/` for `getUTCMonth`/`getMonth()`/`getFullYear()`, for any other `writeHeldDuringMaintenance`/`maintenanceModeActive` callers, and for `Date.now()` usage outside the two files above. `tests/maintenance-mode.test.ts` passes an explicit `at` argument to every call — already date-safe, no change needed. `tests/camp-freshness.test.ts` passes an explicit `now`/`localNow` to every function under test — already date-safe. The remaining `Date.now()` hits (`release-evidence.test.ts`, `offsite-backup.test.ts`, `slack-actions.test.ts`, `slack-lib.test.ts`) all compute a relative offset (`Date.now() + 1 day`, `Date.now() - 600s`) rather than a hardcoded calendar boundary — deterministic regardless of what day it actually is, not the same bug class. No other August-style time bomb found.

### Problem 2: customer-lifecycle still hung 7/8 under `npm run test:integration`

**Diagnosis.** The prior session (above) already isolated `customer-lifecycle.integration.test.ts` and `editorial-records-migration.test.ts` into `vitest.integration.config.ts` (`pool: 'threads'`, `maxWorkers: 1`, `fileParallelism: false`) specifically because each owns a real Miniflare/workerd native subprocess and two files' worth running concurrently crashed the fork. That session's own closing note flagged the risk it didn't rule out: `customer-lifecycle` holds **two** live Miniflare instances at once inside a single file — the `beforeAll` one (alive for the whole file, disposed only in `afterAll`) plus a second one created inside the last test for the commerce sub-scenario. Single-threading stops two *files* from overlapping; it does nothing to stop two workerd processes from *within the same file* being alive simultaneously.

The reported symptom fits that gap precisely: 7 of 8 tests pass, then the file "never completes" rather than erroring. The 8th and last test already has its own `try/finally` disposing the commerce Miniflare (confirmed correct, not the bug), and a 20-second per-test timeout override — so if the hang were a normal async wait, vitest's own timeout should have fired and reported a failure by 20s, not run silently past 47s with no error. That points at a hang inside `dispose()` itself blocking at a level JS timers can't preempt — consistent with the known class of Miniflare/workerd issue on Windows where a second native runtime process, spawned from inside a worker thread that already owns one, doesn't tear down cleanly.

**Fix.** `tests/customer-lifecycle.integration.test.ts`: the last test reads only `commerceDb`, never `db`/`mf`, and it's the last test in the file, so the primary Miniflare instance is now disposed at the very top of that test, before the commerce one is created — never letting two exist at once. Added a `primaryDisposedEarly` flag so `afterAll` doesn't try to dispose it a second time. This removes the overlap directly rather than adding a timeout to catch it after the fact.

`vitest.integration.config.ts`: added explicit `testTimeout: 30_000` / `hookTimeout: 30_000` as a second, independent layer. The vitest defaults (5s/10s) are already narrower than this suite's own per-test overrides (20-30s), so a hang inside a *hook* (`beforeAll`/`afterAll`) had no real backstop before this. This doesn't fix the diagnosed cause on its own — a true native-level block can still outrun a JS timer — but it closes the gap for any hang that is slow-but-real rather than fully blocking, and costs nothing.

**Confidence: moderate, not high.** The two-simultaneous-workerd-processes theory is well-supported (matches the prior session's own flagged risk, matches the 7/8-then-silent-hang shape, matches the confirmed-correct disposal code ruling out a missing try/finally), and the fix removes the overlap outright rather than working around it. But `npx vitest run` cannot execute in this sandbox, so the fix is unverified. If it doesn't hold, the next thing to check is whether `commerceDb`'s own Miniflare instance has some other resource (a listening port, a lock file) contending with the just-disposed primary's residual OS-level cleanup, which would mean the fix needs a small delay after `await mf.dispose()` before creating the second instance, not just reordering.

### What Jeff should check himself

Run `npm test` for real and confirm all 10/10 in the two cron files go green regardless of what day it is. Run `npm run test:integration` and confirm `customer-lifecycle.integration.test.ts` reports 8/8, not 7/8-then-hang; if it still hangs, note whether it now at least times out at 30s with a real error (confirms the config fix helped, points remaining blame at the Miniflare-overlap fix) or hangs indefinitely again (confirms a native-level block the config layer can't catch, and the "small delay" fallback above is the next thing to try).

Files changed: `tests/api/cron-camps-sweep.test.ts`, `tests/worker-cron.test.ts`, `tests/customer-lifecycle.integration.test.ts`, `vitest.integration.config.ts`.
