...
**2026-07-31 attempt:** tried from this session's sandbox (`npx wrangler d1 list`) to confirm reachability before attempting a real time-travel restore into a scratch database. The command returned no output and no error inside a 20-second window — this sandbox has no live Cloudflare API credentials/network path, the same limitation this repo's Pillar 9 notes document for `astro build`/`vitest`. Row stays failing, honestly, not marked done. **Human action needed:** from a machine with the real `CLOUDFLARE_API_TOKEN`, create a throwaway D1 database (`npx wrangler d1 create activity-radar-restore-test`), run `npx wrangler d1 time-travel restore activity-radar-restore-test --timestamp=<iso-timestamp-from-24h-ago>`, verify a known row count, log the result and date in this table, then delete the scratch database.
