...
export function sanitizeExternalMarkdown(markdown: string): string {
  ...
  // Provider copy often closes with a personal Jeff sign-off or a "Note From
  // Jeff" heading. Public brand is Parent Coach Desk editorial — no personal
  // byline — so strip those before the body is written to src/content.
  value = stripPersonalJeffNaming(value);
  value = value.replace(/\r\n/g, '\n').replace(/\n{4,}/g, '\n\n\n').trim();
  return `${value}\n`;
}
...
function rewriteJeffPublicLabel(text: string): string {
  return text
    .replace(/\bHow Jeff Applies These Scripts as a Parent-Coach\b/gi, 'How a Parent-Coach Applies These Scripts')
    .replace(/\bHow Jeff Applies\b/gi, 'How a Parent-Coach Applies')
    .replace(/\bHow Jeff\b/gi, 'How a Parent-Coach')
    .replace(/\bA Note From Jeff\b/gi, 'A Note')
    .replace(/\bFrom Jeff\b/gi, '')
    .replace(/\bJeff Thomas\b/gi, 'Parent Coach Desk')
    .replace(/\s{2,}/g, ' ')
    .trim();
}

function stripPersonalJeffNaming(markdown: string): string {
  let value = markdown;
  value = value.replace(/^(?:>\s*)?(?:\*|_)*\s*[—–-]\s*Jeff(?:\s+Thomas)?\s*(?:\*|_)*\s*$/gim, '');
  value = value.replace(/^(?:>\s*)?(?:\*|_)*\s*Written by Jeff(?:\s+Thomas)?\.?\s*(?:\*|_)*\s*$/gim, '');
  value = value.replace(/^(#{1,6}\s+)(.+)$/gm, (_match, hashes: string, title: string) => {
    return `${hashes}${rewriteJeffPublicLabel(title)}`;
  });
  value = value.replace(/\[([^\]]*?\bJeff\b[^\]]*?)\]\((#[^)]+)\)/g, (_match, label: string) => {
    const rewritten = rewriteJeffPublicLabel(label);
    return `[${rewritten}](#${headingSlug(rewritten)})`;
  });
  return value;
}
