...
// Domain errors authored in src/lib/editorial-records.ts (addSource). These
...
const KNOWN_ERRORS = [
  ...
  'source quality must be an integer 0-100',
  'content sha256 must be 64 hex characters',
];
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const logger = createRequestLogger(request, { route: 'admin/editorial/sources/create', userId: auth.email });
  ...
  try {
    ...
    logger.error('editorial_record_write_failed', error, { entityType: body.entity_type, entityId: body.entity_id });
