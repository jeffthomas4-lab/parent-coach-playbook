...
| Editorial refresh discipline | Implemented and verified locally | A release-gated snapshot applies the business plan's 18-month trigger to all currently live articles, excludes drafts/future pieces, tracks fact-check and affiliate-review context, and surfaces aggregates on the protected editorial desk. It cannot infer decommissioning without live traffic, emitted-link evidence, supersession review, and a human decision. |
...
| Post-deploy health evidence | Implemented and verified locally | `f8e1bfb` covers public HTML, camps, health, readiness, static assets, non-mutating API, and Access redirect. Each sanitized staging and production smoke receipt now receives a detached SHA-256 sidecar that is verified before both files are retained together. |
| Exact deployed asset/version-skew gate | Implemented and verified locally | Each build records one content-hashed Astro asset's path, byte length, SHA-256, and Git SHA; smoke fails unless the deployed bytes match, and the sanitized receipt embeds the same immutable identity. Gradual or mixed-version delivery remains prohibited pending a separate design and rehearsal. |
