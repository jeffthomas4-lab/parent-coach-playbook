...
- A fresh authenticated R2 dashboard inventory shows the production `PHOTOS` binding bucket (`activityradar-photos`) is empty: zero objects and zero bytes. No production R2 payload needs copying today, but this does not become an `not_applicable` release-gate exception without Jeff's explicit owner decision. See `coordination/release-evidence/production-r2-inventory-preflight-2026-07-17.json`.
