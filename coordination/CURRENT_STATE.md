...
## 2026-07-18 - deploy hardening: auto-remediation, S4 freeze exemption, additive verifiers

- **Q5 auto-remediation (implemented and verified locally):** the deploy workflow ran its post-deploy smoke after `wrangler deploy` had already put the version live, so a failed smoke only turned the run red while the broken version kept serving. `scripts/deploy-remediation.mjs` now rolls production traffic back to the recorded good version id on smoke failure, or halts with a loud alert when no valid rollback target exists or the rollback itself fails. Staging halts and alerts. Both jobs invoke it only when the deploy step succeeded and the smoke step failed. Six focused tests drive rollback, halt, rollback-failure, and no-op paths against a mocked Wrangler CLI.
- **Q3 S4 freeze exemption (implemented and verified locally):** the S4 deletion opt-out is now a code-enforced exempt write class in `src/lib/maintenance-mode.ts` (`writeHeldDuringMaintenance`, `MAINTENANCE_EXEMPT_WRITE_CLASSES`). It is never held from August 1 through November 30 and the operator override cannot suppress it. The camps-sweep cron guard routes through the same function with no behavior change. Three required tests plus exemption coverage pass.
- **Q3 enforcement finding (documented):** the freeze is code-enforced only where a write flows through code, which is Ranger's S7 camps-sweep. Vera's deletion path does not route through the protected site mutation endpoints; those admin routes gate on admin auth plus same-origin and never call the maintenance guard. The other eight tasks' report-only posture during the freeze is prompt-only. This is recorded in `PCD-AI-OS/00-FOUNDATIONS.md` item 10, and clarifies the earlier phrase "protected site mutation endpoint" in the seasonal-maintenance entry below: only the cron sweep path is code-enforced.
- **Q1/Q2 additive verifiers (implemented and verified locally):** `scripts/smoke-attestation.mjs` accepts, or in require mode demands, an independently issued GitHub Actions OIDC artifact attestation of the smoke receipt and checksum pair, with no new secret. `scripts/caller-hashes.mjs` records a SHA-256 per committed scheduled-task caller and matches it against `coordination/release-evidence/scheduled-task-caller-hashes.json`, turning reconciliation proof step 2 into a hash match. The caller-hash check is now part of `ci:release`. Fourteen focused tests pass.
- **Verified locally:** rebased onto `0ccd55f` ("Build governed affiliate recommendation lifecycle"). Full Vitest suite 167 files / 843 tests; `astro check` 0 errors, 0 warnings, 356 hints; the affected inventory checks pass (`check:caller-hashes`, `check:affiliate-lifecycle`, `check:agent-token-rollout`, `check:workflow-pins`, `check:rollback-target`); `check:root-organization` 85 artifacts and 36 directories. Base `0ccd55f` alone was 165 files / 819 tests. The production build compiles clean and was run to completion on identical slice code at `e808801`; the final build runs on the landing machine before the gated deploy.
- **Claim boundary:** all four slices are local code and tests only. No deployment, rollback rehearsal, migration, credential, or provider action was taken. The live staging rollback drill and any production deploy remain owner-gated per `coordination/JEFF-ACTION-PACKAGE-2026-07-18.md`.
...
- **Implemented locally:** `/camps/verification/` now explains the existing public `Verified` badge: minimum evidence, recorded review date, explicit non-endorsement limits, freshness triggers, moderated correction/removal workflow, and commercial separation. Directory badges and verified detail pages link to the methodology. Camp verification is explicitly distinct from the product mark.
...
# 2026-07-18 - distinct Parent Coach Approved product-mark foundation

- **Implemented locally:** `/parent-coach-approved/` now publishes the product-mark methodology required before any badge can exist. A versioned, empty-by-default registry enforces at least 12 weeks of use, two distinct alternatives, non-empty wear evidence, an honest verdict, who the product is wrong for, when to skip the category, explicit human approval, and a valid approval date. Paid placement is incompatible with the mark, and the public page states that no product has been awarded it.
- **Verified locally:** focused product-mark, camp-separation, and route-governance coverage passes 3 files / 9 tests. The generated route-control inventory classifies all 139 routes, the production build and built-link contract pass, Astro diagnostics report 0 errors, 0 warnings, and 356 existing hints, and the serialized full suite passes 161 files / 809 tests.
- **External boundary:** no product was tested, approved, badged, promoted, purchased, or submitted for manufacturer response. The registry remains in `foundation_no_awards`, and no external message, provider action, deployment, or commerce mutation occurred.
...
# 2026-07-18 - governed editorial refresh queue

- **Implemented locally:** the business plan's 18-month article-refresh rule is now a deterministic release-gated snapshot over the current live article collection. It excludes drafts and future-scheduled pieces, records refresh and fact-check dates, flags affiliate-bearing articles for link review, and surfaces aggregate status on the protected editorial desk.
- **Current evidence:** the 2026-07-18 snapshot contains 804 live articles, 0 refresh-due articles, 0 fact-check-due articles, and 72 affiliate-bearing articles. Literal contextual source references are recorded only as a supporting signal; generated hub links require emitted-artifact evidence and are not mislabeled as missing.
- **Verified locally:** focused refresh, protected-route, and route-control coverage passes 2 files / 7 tests; `check:editorial-refresh` proves the committed snapshot matches source; Astro diagnostics report 0 errors, 0 warnings, and 356 existing hints; and the complete verbose single-worker suite passes 158 files / 800 tests.
- **Governance boundary:** the queue contains no live traffic or external analytics data and cannot redirect, delete, publish, or edit content. Decommissioning still requires reviewed live traffic below the business-plan threshold, an emitted incoming-link audit, supersession evidence, and a human decision.
# 2026-07-18 - consented newsletter signup intent

- **Implemented locally:** every shipped direct link to the governed Kit signup destination now carries one allowlisted placement label. After analytics consent—and only while the existing GA boundary is active—the site emits `newsletter_signup_intent` with that placement and a beacon transport hint.
- **Verified locally:** focused newsletter and affiliate analytics coverage passes 2 files / 5 tests; Astro diagnostics report 0 errors, 0 warnings, and 356 existing hints; and the complete verbose single-worker suite passes 159 files / 803 tests.
- **Privacy and truth boundary:** the event excludes email, subscriber identity, destination URL, link text, query/fragment data, cookies, and provider outcome. It never delays navigation and does not claim subscription, confirmation, delivery, or list growth. DNT/GPC, denied consent, EU geofencing, and provider lifecycle proof remain governed by their existing stricter controls.
# 2026-07-18 - evidence-bounded camp Event schema

- **Corrected locally:** the camp detail route previously contradicted its own comment by emitting `Event` JSON-LD for legacy approved rows without a start date and represented the program name as an organizer without a separately verified organization name. Event schema now emits only for valid, ordered start/end dates and a complete US postal location; it omits organizer, offers, contact data, and unsafe photo keys.
- **Verified locally:** focused Event/JSON-LD coverage passes 2 files / 6 tests; Astro diagnostics report 0 errors, 0 warnings, and 356 existing hints. The first complete-suite attempt ended with the recurring Vitest fork exit and is not counted as green; an immediate clean rerun passes 160 files / 806 tests.
- **Truth boundary:** rows with incomplete event evidence still render their honest public listing but emit no Event claim. The builder does not infer an organizer, price, availability, accreditation, verification, or endorsement.
# 2026-07-18 - consolidated company operating system

- **Implemented locally:** `strategy/PARENT-COACH-PLAYBOOK-COMPANY-OPERATING-SYSTEM.md` is the single human-readable company operating authority. It seamlessly consolidates the 24-month business plan, discovery portfolio, content and product system, four revenue lines, founder/staffing model, AI governance, operating cadence, scorecard, maintenance and resilience, risk system, experiment rules, current implementation boundary, and 90-day execution plan. The original business plan, detailed operating manual, AI-OS foundations, content roadmap, and monetization plan now point to the master while remaining specialist evidence.
- **Made executable:** `strategy/company-operating-system.json` defines the four milestone gates, 21 metric definitions, 12 standing risks, 15–20 hour founder-capacity boundary, protected priorities, approval classes, and default-off customer/commerce state. `check:company-os` is a release gate and rejects manufactured actuals, changed milestone decisions without governed review, weakened capacity limits, or unauthorized commerce activation. `templates/COMPANY-OPERATING-REVIEW.md` provides one weekly/monthly/quarterly review form.
- **Document artifact:** `strategy/Parent_Coach_Playbook_Company_Operating_System.docx` is generated deterministically from the canonical Markdown using the compact reference-guide design system. Structural QA reports correct Letter geometry, 18 Heading 1 and 47 Heading 2 paragraphs, and zero accessibility findings. Page-image render QA could not run because LibreOffice is unavailable in this environment.
- **Verified locally:** focused company-OS and organization coverage passes 2 files / 5 tests; the standalone company-OS contract passes with 21 metrics and 12 risks. The serialized full suite passes 162 files / 812 tests, and Astro diagnostics report 0 errors, 0 warnings, and 356 existing hints.
- **External boundary:** all live analytics actuals remain pending rather than fabricated. No provider, account, credential, deployment, message, post, outreach, customer, commerce, legal, tax, insurance, or accessibility action was performed.

# 2026-07-18 - editorial opportunity lifecycle durable records

- **Verified in code:** migration `0024_editorial_opportunity_lifecycle.sql` (10 tables: opportunities, an append-only lifecycle-event log, an evidence-source registry, briefs, claims, claim-source links, relationships, editorial/SEO reviews, approvals, maintenance proposals) plus `src/lib/editorial-records.ts`, which routes every transition through the existing `canTransition`/`approvalGates` state machine in `src/lib/editorial-lifecycle.ts` rather than reimplementing it. `src/lib/editorial-opportunity-intake.ts` adds a write-time PII/URL/phone redaction gate and two real adapters (the existing `demand_events_v1` aggregate and the public corrections log); the remaining seven opportunity-intake sources throw a named `NotImplementedAdapterError` instead of fabricating signals. Migration `0024` is committed local design only, exactly like `0023`: not applied to staging or production.
- **Verified by tests:** a disposable-D1 Miniflare integration suite (`tests/editorial-records-migration.test.ts`, 9/9) walks a full opportunity through discovery, scoring, classification, briefing, claim validation, editorial/SEO review, relationship mapping, monetization classification, and human approval, plus the insufficient-evidence stop and the propose-but-never-execute-retirement path. 77 additional unit tests (`tests/editorial-opportunity-intake.test.ts`, 9 `tests/api/admin-editorial-*.test.ts` files) cover the intake sanitizer and every new admin route's auth/origin/feature-flag/validation boundary. The full repo unit suite passes 834/837 (see below for the 3 known exceptions); the full integration suite passes 53/53; `astro check` reports 0 errors, 0 warnings, 356 hints, matching the documented baseline exactly.
- **Access control:** 8 new admin routes under `src/pages/api/admin/editorial/{opportunities,sources,briefs,claims,reviews,relationships,approvals,maintenance}/`, each behind Cloudflare Access (`requireAdmin`), same-origin (`requireSameOrigin`), and a new default-off flag `EDITORIAL_LIFECYCLE_ENABLED` (added to both wrangler configs and to the production-manifest safe-default release gate). The protected-route contract grew from 38 to 48 routes; `npm run check:protected-routes` passes at 48.
- **Known consequence, not fixed here:** the last live anonymous Access probe (`tests/anonymous-admin-probe.test.ts`) and the authenticated Access evidence packet are now stale against the 48-route surface, since 10 real new protected routes exist that have not been live-probed. A fresh live probe is a production HTTP action requiring separate approval; it was not performed. Separately, `coordination/release-evidence/authenticated-access-probes-pending.json` was confirmed (via `git show` against baseline commit `192a173`) to already contain `"state": "complete"` despite its filename before this session began -- a pre-existing inconsistency, not introduced here.
- **Governance:** `coordination/plans/020-editorial-opportunity-lifecycle-migration.md` and `coordination/release-evidence/editorial-lifecycle-migration-rehearsal-2026-07-18.json` record the migration plan and local-only rehearsal evidence (`check:editorial-lifecycle-migration-rehearsal` passes). `strategy/EDITORIAL-CORPUS-RECONCILIATION-BATCHING.md` designs the legacy-corpus reconciliation batching approach (pillar collection, 17 items, proposed as the pilot) without reconciling any item; `scripts/editorial-reconciliation-pilot-candidates.mjs` is a read-only report, not a mutation.
- **External boundary:** no staging or production migration, no feature-flag change, no external provider call, no publish/delete/redirect action, no affiliate change, no live production request, and no legacy-corpus item's status changed. Full session detail: `coordination/handoffs/2026-07-18-editorial-lifecycle-durable-records-claude-code.md`.
