...
describe('POST /api/admin/editorial/reviews/create', () => {
  ...
  it('records the reviewer as the authenticated actor, never a client-supplied identity', async () => {
    ...
    expect(records.recordReview).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ reviewer: 'admin-fixture@parentcoachdesk.com', actor: 'admin-fixture@parentcoachdesk.com' }));
  });
