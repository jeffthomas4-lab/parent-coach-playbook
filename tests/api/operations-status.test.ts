...
describe('operations status', () => {
  ...
  it('distinguishes healthy supply, stale sweeps, and failing agent runs', async () => {
    const rows = await readOperationsStatus({
      DB: db([
        ...
        { approved: 25, age_missing: 2, age_impossible: 0, dates_missing: 0, dates_reversed: 0, negative_price: 0, registration_url_missing: 0, registration_url_non_https: 0, source_domain_missing: 0, exact_duplicate_excess_records: 1, verified_without_timestamp: 0, verified_future_timestamp: 0, verified_with_evidence: 10, verification_review_current: 8, verification_review_due: 2, distinct_source_domains: 7, oldest_verified_at: '2026-07-01T00:00:00Z', latest_verified_at: '2026-07-15T00:00:00Z', url_never_checked: 2 },
  ...
  it('degrades directory quality when conservative exact duplicate candidates exist', async () => {
    const rows = await readOperationsStatus({
      DB: db([
        { n: 2 },
        { approved_records: 2, never_checked: 0, last_checked_at: '2026-07-16T00:00:00.000Z' },
        { approved: 2, exact_duplicate_excess_records: 1 },
        { pending: 0, oldest: null }, { pending: 0, oldest: null },
        { pending: 0, oldest: null }, { pending: 0, oldest: null },
      ]),
    }, new Date('2026-07-16T01:00:00.000Z'));
    expect(rows).toEqual(expect.arrayContaining([
      expect.objectContaining({ component: 'Directory data quality', state: 'degraded', code: 'approved_data_incomplete', metrics: expect.objectContaining({ exact_duplicate_excess_records: 1 }) }),
    ]));
  });
