...
describe('live Access evidence contracts', () => {
  it('accepts pending packets without confusing them for completed evidence', () => {
    expect(validateAccessPolicyEvidence(policy)).toEqual({ errors: [], valid: true, complete: false });
  ...
  it('requires full allowed and denied route coverage without mutation', () => {
    ...
    expect(validateAuthenticatedAccessEvidence(complete).errors).toEqual(expect.arrayContaining([
      'allowed_results must cover all 37 routes', 'denied_results must cover all 37 routes',
    ]));
  });
});
