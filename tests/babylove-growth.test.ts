...
`);
  ...
  it('strips personal Jeff sign-offs and rewrites Jeff-named headings to brand voice', () => {
    const safe = sanitizeExternalMarkdown(`${LONG_BODY}
...
One cue per practice.

— Jeff Thomas
`);

    expect(safe).not.toMatch(/\bJeff\b/);
    expect(safe).toContain('[A Note on Coaching Your Own Kid in Goal](#a-note-on-coaching-your-own-kid-in-goal)');
    expect(safe).toContain('## A Note on Coaching Your Own Kid in Goal');
    expect(safe).toContain('## How a Parent-Coach Applies These Scripts');
    expect(safe).toContain('Set the expectation early.');
    expect(safe).toContain('One cue per practice.');
  });

  it('leaves Jefferson place names intact', () => {
    const safe = sanitizeExternalMarkdown(`${LONG_BODY}\n\nThe tournament is at Jefferson Park.\n`);
    expect(safe).toContain('Jefferson Park');
